...
export async function GET() {
  try {
    ...
    const potentialTypes: Record<string, { description: string; suggestion: string[] }> = {
      ...
      '主动探索型': {
        description: '主动性强，善于发现问题并寻求解决方案',
        suggestion: [
          '让其参与跨部门讨论',
          '鼓励输出方法沉淀',
          '安排展示机会',
    ...
    const formattedInterns = interns.map(intern => {
      ...
      if (intern.scoreHistory.length >= 2) {
        ...
        if (fitChange > 5 && potentialChange > 5) {
          growthTrend = 'rising';
        } else if (fitChange < -5 || potentialChange < -5) {
      ...
      // 生成推荐理由
      const reasons = [];
      if (intern.taskCompletionRate >= 90) {
        reasons.push('任务完成率稳定在90%以上');
      }
      if (intern.weeklyReports[0]?.emotionSignal === '积极') {
        reasons.push('周报表达积极主动');
      }
      if (intern.mentorFeedbacks[0]?.performance >= 4) {
        reasons.push('导师给予高度评价');
      }
      if (growthTrend === 'rising') {
        reasons.push('近期成长趋势明显');
      }
      if (reasons.length === 0) {
        reasons.push('综合表现优秀');
      }
      ...
      return {
        ...
        potentialType: intern.potentialType || '综合优秀型',
    ...
    return NextResponse.json({
      ...
      typeDescriptions: potentialTypes,
    });
  } catch (error) {
